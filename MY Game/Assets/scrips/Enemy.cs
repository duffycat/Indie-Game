...
public class EnemyStunState : EnemyBehaviourState
{
    ...
    public override void OnStateEnter()
    {
        ...
        Instance.Anim.SetBool("walking", false);
        Instance.Anim.SetBool("chase", false);
        Instance.Anim.SetBool("stun", true);
        Instance.Agent.isStopped = true;
        stunTimer = stunTime;
    ...
    public override void OnStateExit()
    {
        timer = -1;
        stunTimer = 0;
        Instance.Anim.SetBool("stun", false);
    ...
    public override void OnStateUpdate()
    {
        if (timer >= 0)
        {
            ...
            if (timer >= stunTimer)
            {
                ...
                if (Instance.TrueChase == true)
                {
                    Instance.SetState(Instance.gameOnState);
                }
                else 
                {
                    Instance.SetState(Instance.wanderState);
